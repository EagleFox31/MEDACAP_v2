                            ...
                            <table aria-describedby=""
                                ...
                                <thead>
                                    <tr class="text-center text-black fw-bold fs-7 text-uppercase gs-0">
                                        ...
                                        <?php
                                            ...
                                            $trainingTypes = ['Présentielle', 'Distancielle', 'E-learning', 'Coaching', 'Mentoring'];
                                ...
                                <tbody class="fw-semibold text-gray-600" id="table">
                                    ...
                                    foreach ($functionalGroups as $group) {
                                        ...
                                        // Début de la ligne pour le groupe fonctionnel
                                        ...
                                        // Catégoriser les scores
                                        $factuelCategory = ($factuelScore !== 'N/A') ? ( $factuelScore >= 80 ? 'high' : ($factuelScore >= 60 ? 'mid' : 'low') ) : 'N/A';
                                        $declaratifCategory = ($declaratifScore !== 'N/A') ? ( $declaratifScore >= 80 ? 'high' : ($declaratifScore >= 60 ? 'mid' : 'low') ) : 'N/A';

                                        // Déterminer l'affichage des badges
                                        $displayCheckmark = false;
                                        if ($factuelCategory === 'high' && $declaratifCategory === 'high') {
                                        ...
                                        if ($isGrayedOut) {
                                            echo '<td class="text-center">-</td>';
                                             ...
                                             // Affichage du badge Factuel
                                            echo '<td class="text-center">';
                                            if ($displayCheckmark) {
                                                echo '<span class="badge ' . getBootstrapClass($factuelScore) . '" data-toggle="tooltip" title="Score Factuel : ' . round($factuelScore) . '%">' . round($factuelScore) . '%</span>';
                                            } else {
                                                if ($factuelScore === 'N/A') {
                                                    echo '<span class="badge badge-secondary" data-toggle="tooltip" data-placement="top" title="' . htmlspecialchars($technician['firstName']) . ' n\'a pas été évalué dans le groupe fonctionnel ' . htmlspecialchars($group) . ' au niveau ' . htmlspecialchars($selectedLevel) . ' (Connaissances).">N/A</span>';
                                                } else {
                                                    echo '<span class="badge ' . getBootstrapClass($factuelScore) . '" data-toggle="tooltip" title="Score Factuel : ' . round($factuelScore) . '%">' . round($factuelScore) . '%</span>';
                                                }
                                            }
                                        ...
                                        foreach ($trainingTypes as $type) {
                                            foreach ($validBrands as $brand) {
                                                ...
                                                if (isset($trainingsByBrandAndType[$type][$brand]) && !empty($trainingsByBrandAndType[$type][$brand])) {
                                                    foreach ($trainingsByBrandAndType[$type][$brand] as $training) {
                                                        // Convertir $training['speciality'] en tableau
                                                        $specialityArray = ($training['speciality'] instanceof MongoDB\Model\BSONArray) ? $training['speciality']->getArrayCopy() : (array)$training['speciality'];
                                                        if (in_array($group, $specialityArray)) {
                                                            $c_hours = isset($training['c_hours']) ? intval($training['c_hours']) : null;
                                                            $tp_hours = isset($training['tp_hours']) ? intval($training['tp_hours']) : null;
                                                           // Déterminer l'heure à afficher
                                                            $hoursToDisplay = null;
                                                            if ($c_hours !== null && $tp_hours !== null) {
                                                                $hoursToDisplay = ($c_hours >= $tp_hours) ? $c_hours : $tp_hours;
                                                            } elseif ($c_hours !== null) {
                                                                $hoursToDisplay = $c_hours;
                                                            } elseif ($tp_hours !== null) {
                                                                $hoursToDisplay = $tp_hours;
                                                            }

                                                            if ($hoursToDisplay !== null && $hoursToDisplay > 0) {
                                                                echo '<td class="text-center">
                                                                        <span class="badge badge-primary hour-display" 
                                                                              data-training-id="' . htmlspecialchars($training['_id']) . '" 
                                                                              data-group="' . htmlspecialchars($group) . '" 
                                                                              data-type="c" 
                                                                              data-toggle="tooltip" 
                                                                              title="Heures de formation recommandées : ' . $hoursToDisplay . 'h">' 
                                                                              . htmlspecialchars($hoursToDisplay) . 'h
                                                                        </span>
                                        ...
                                        if ($isGrayedOut) {
                                            ...
                                            echo '<td class="text-center">';
                                            if ($displayCheckmark) {
                                                echo '<span class="badge ' . getBootstrapClass($declaratifScore) . '" data-toggle="tooltip" title="Score Declaratif : ' . round($declaratifScore) . '%">' . round($declaratifScore) . '%</span>';
                                            } else {
                                                if ($declaratifScore === 'N/A') {
                                                    echo '<span class="badge badge-secondary" data-toggle="tooltip" title="' . htmlspecialchars($technician['firstName']) . ' n\'a pas été évalué dans le groupe fonctionnel ' . htmlspecialchars($group) . ' au niveau ' . htmlspecialchars($selectedLevel) . ' (Tâches Professionnelles).">N/A</span>';
                                                } else {
                                                    echo '<span class="badge ' . getBootstrapClass($declaratifScore) . '" data-toggle="tooltip" title="Score Declaratif : ' . round($declaratifScore) . '%">' . round($declaratifScore) . '%</span>';
                                                }
                                            }
...
$(document).ready(function () {
    ...
    $(document).ready(function() {
        ...
        // Fonction pour ouvrir la dropdown lors du clic sur le badge
        ...
        // Générer les dropdowns dynamiquement pour chaque badge .hour-display
        $('.hour-display').each(function() {
            ...
            for (var i = 5; i <= 40; i += 5) {
                ...
                a.click(function(e) {
                    e.preventDefault();
                    var selectedHours = parseInt($(this).text().replace('h', ''));
                    var badge = $(this).closest('.dropdown-menu').siblings('.hour-display');
                    // Mettre à jour l'affichage dans le badge
                    badge.text(selectedHours + 'h');
                    // Masquer la dropdown
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (res.status === 'success') {
                                ...
                                // Afficher une notification d'avertissement ou d'erreur
                                toastr.warning(res.message, 'Attention');
                            }
                        },
                        error: function(xhr, status, error) {
                            console.error(error);
                            // Afficher une notification d'erreur
                            toastr.error('Erreur lors de la mise à jour des heures.', 'Erreur');
                        }
                    });
                });
                li.append(a);
            ...
            // Ajouter la dropdown après le badge
            $(this).after(dropdown);
        });
