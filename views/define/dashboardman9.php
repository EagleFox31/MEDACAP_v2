...
if (!isset($_SESSION["profile"])) {
                        ...
                        foreach ($agency['managers'] as $manager) {
                            ...
                            if (isset($manager['technicians'])) {
                                foreach ($manager['technicians'] as $technician) {
                                    ...
                                    // 1) Récupérer le document user dans $usersColl
                                    $userDoc = null;
                                    if (!empty($techId)) {
                                    ...
                                    // 2) Extraire le level depuis userDoc (ou 'N/A' s'il n'existe pas)
    ...
    if ($filterTechnician === 'all') {
        // --- GROUPE DISTINCT ---
        // 1) Group by brand, level, training._id
        $pipeline2[] = [
            '$group' => [
                '_id' => [
        ...
        $pipeline2[] = [
            '$group' => [
                ...
                // on compte 1 par trainingId distinct
                'count' => ['$sum' => 1]
            ]
        ];
    } else {
    ...
    if ($subsidiary === "CFAO MOTORS GABON" && ($managerId === "all" || $managerId === "6658c6b6596e00007000224d")  && $filterTechnician === "all") {
        if ($filterLevel === 'all') {
            $trainingsCountsForGraph2 = [
                'RENAULT TRUCK'     => 9,   // Total 9
                'FUSO'              => 7,   // Total 7
                'KING LONG'         => 3,   // Total 3
                'JCB'               => 9,   // Total 9
                'SINOTRUK'          => 6,   // Total 6
                'TOYOTA BT'         => 3,   // Total 3
                'TOYOTA FORKLIFT'   => 11   // Total 11
            ];
        } elseif ($filterLevel === 'Junior') {
            $trainingsCountsForGraph2 = [
                'RENAULT TRUCK'     => 1,   // 1 Junior
                'FUSO'              => 1,   // 1 Junior
                'KING LONG'         => 1,   // 1 Junior
                'JCB'               => 4,   // 4 Junior
                'SINOTRUK'          => 1,   // 1 Junior
                'TOYOTA BT'         => 0,   // 0 Junior
                'TOYOTA FORKLIFT'   => 3    // 3 Junior
            ];
        } elseif ($filterLevel === 'Senior') {
            $trainingsCountsForGraph2 = [
                'RENAULT TRUCK'     => 6,   // 6 Senior
                'FUSO'              => 5,   // 5 Senior
                'KING LONG'         => 2,   // 2 Senior
                'JCB'               => 5,   // 5 Senior
                'SINOTRUK'          => 5,   // 5 Senior
                'TOYOTA BT'         => 3,   // 3 Senior
                'TOYOTA FORKLIFT'   => 6    // 6 Senior
            ];
        } elseif ($filterLevel === 'Expert') {
            $trainingsCountsForGraph2 = [
                'RENAULT TRUCK'     => 2,   // 2 Expert
                'FUSO'              => 1,   // 1 Expert
                'KING LONG'         => 0,   // 0
                'JCB'               => 0,   // 0
                'SINOTRUK'          => 0,   // 0
                'TOYOTA BT'         => 0,   // 0
                'TOYOTA FORKLIFT'   => 2    // 2 Expert
            ];
        }
        // Filtrer par marque si nécessaire
        // Filtrer par marque si nécessaire
        if ($filterBrand !== 'all') {
            if (isset($trainingsCountsForGraph2[$filterBrand])) {
                // Extraire seulement la marque filtrée
                $trainingsCountsForGraph2 = [
                    $filterBrand => $trainingsCountsForGraph2[$filterBrand]
                ];
            } else {
                // Si la marque n'est pas trouvée, définir le count à 0
                $trainingsCountsForGraph2 = [
                    $filterBrand => 0
                ];
            }
                    ...
                    <div class="container-xxl">
                        ...
                        <script>
                            ...
                            function updateLevelOptions(selectedTechId) {
                                ...
                                // 1) Vider toutes les <option> du select
                                levelFilter.innerHTML = '';

                                // 2) Lire la valeur "level" depuis l'URL si elle existe
                                //    (Ex: ?level=Senior => chosenLevel = "Senior")
                                const urlParams = new URLSearchParams(window.location.search);
                                const chosenLevel = urlParams.get('level') || 'all'; // Par défaut 'all' si absent

                                // 3) Si selectedTechId == "all" ou vide => afficher TOUT
                                if (selectedTechId === 'all' || !selectedTechId) {
                                    // Recréer toutes les options (all, Junior, Senior, Expert)
                                    allLevelOptions.forEach(opt => {
                                        const option = new Option(opt.text, opt.value);
                                        levelFilter.add(option);
                                    });
                                    // Tenter de sélectionner chosenLevel s'il existe dans la liste
                                    levelFilter.value = chosenLevel;
                                    return;
                                }

                                // 4) Sinon, on cherche le technicien dans variablesPHP.technicians
                                const tech = variablesPHP.technicians.find(t => t.id === selectedTechId);
                                if (!tech) {
                                    // Si le technicien n'est pas trouvé => tout
                                    allLevelOptions.forEach(opt => {
                                        const option = new Option(opt.text, opt.value);
                                        levelFilter.add(option);
                                    });
                                ...
                                // 5) On a un technicien précis => restreindre les niveaux
                                //    Toujours proposer "all"
                                const allOption = new Option('Tous', 'all');
                                levelFilter.add(allOption);

                                // On récupère le level "réel" du technicien
                                const techLevel = tech.level; // "Junior", "Senior", "Expert", ...

                                // Ajout conditionnel
                                if (techLevel === 'Junior') {
                                    levelFilter.add(new Option('Junior', 'Junior'));
                                } else if (techLevel === 'Senior') {
                                    levelFilter.add(new Option('Junior', 'Junior'));
                                    ...
                                    // Niveau inconnu => on remet tout
                                    allLevelOptions.forEach(opt => {
                                        levelFilter.add(new Option(opt.text, opt.value));
                                    });
                                }

                                // 6) Enfin, tenter de sélectionner chosenLevel
                                levelFilter.value = chosenLevel;
                            }
                            ...
                            document.addEventListener('DOMContentLoaded', () => {
                                const selectedTechId = document.getElementById('technician-filter').value;
                                updateLevelOptions(selectedTechId);
                            });
                        </script>
                        ...
                        <div class="chart-dashboard-container">
                            ...
                            <div class="row mb-4">
                                <div class="col-12">
                                    <div class="card custom-card">
                                        <div class="card-body">
                                            <h3 class="text-center mb-4">2. Plans de Formations Collectifs de la Filiale par Marque</h3>
                                            <div class="scrollable-chart-container">
