...
if (!isset($_SESSION["id"])) {
    ...
    // ---------------------------------------------------------------------------
//  NOTIFICATION « QCM Tâche Professionnelle terminé par le technicien »
//  ─ Destinataires : ADMIN(S)            ─ Cc : statique + manager + RH + DPS
// ---------------------------------------------------------------------------
if (isset($technician['firstName'], $technician['lastName'], $level)) {

    /* 1️⃣  Administrateur(s) de la filiale / dept / agency */
    $adminDocs = getAdminsDocs($technician, $mongo);   // helper du mail_helpers.php
    if (!$adminDocs) {
        header("Location: ./congrat");                 // rien à envoyer
        return;
    }
    ...
    $subject = sprintf(
        'QCM Tâche Professionnelle terminé par le Technicien %s %s pour le Niveau %s',
    ...
    $body = sprintf(
        '<p>%s</p>
         <p>Le QCM Tâche Professionnelle vient d’être terminé par le technicien
         <strong>%s&nbsp;%s</strong> au niveau <strong>%s</strong>. Vous pouvez consulter
         son résultat dans l’espace <em>Résultats des Techniciens par niveau</em>
         de la plateforme MEDACAP.</p>',
        $salutation,
